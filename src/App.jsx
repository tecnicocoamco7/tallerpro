...
function Equipos({ setModulo }) {
  const { equipos, setEquipos, clientes, estadosEquipo, setEstadosEquipo, tiposEquipo, setTiposEquipo, user, addLog } = useApp();
  ...
  const filtrados = equipos.filter(eq=>{
    ...
    return ok&&(filtroTipo==="todos"||eq.tipo===filtroTipo)&&(filtroEst==="todos"||eq.estado===filtroEst);
  });
...
function PerfilEquipo({ equipoId, onVolver, setModulo }) {
  const { equipos, setEquipos, clientes, estadosEquipo, setEstadosEquipo, tiposEquipo, ordenesTrabajos, setOrdenesTrabajo, user, addLog, notificaciones, setNotificaciones, usuarios } = useApp();
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      supabase.from(STATE_TABLE).upsert({
        id: STATE_ROW_ID,
        data: { clientes, equipos, ordenesTrabajos, log, tiposEquipo, estadosEquipo, notificaciones, etiquetasCliente },
        updated_at: new Date().toISOString(),
      }).then(({ error }) => { if (error) console.error("Error guardando en Supabase:", error); });
    }, 600);
    return () => clearTimeout(t);
  }, [clientes, equipos, ordenesTrabajos, log, tiposEquipo, estadosEquipo, notificaciones, etiquetasCliente, loaded]);
